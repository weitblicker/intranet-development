...
# ---------------------------------------------
# sync O365 -> securitygroup

# $cities = "muenster"  # only execute single city
...
foreach($city in $cities){
  ...
  # mitgliedsgruppe -> mitglieder
  $i = 0
  foreach($user in $sec_member) {
    if($o_member -notcontains $user){
      # user was removed since last sync
      "remove " + $user
      Remove-DistributionGroupMember $sec_member_address -Member $user -Confirm:  $false -BypassSecurityGroupManagerCheck
      $i += 1
    }
  }
  ...
  foreach($user in $o_member) {
      if($sec_member -notcontains $user){
        ...
        "add " + $user
        Add-DistributionGroupMember $sec_member_address -Member $user -BypassSecurityGroupManagerCheck
        $j += 1
      }
  }
  Write-Host $("mitglieder:  removed = " + $i + "    added = " + $j)

  # vorstandsgruppe -> vorstand
  $n = 0
  foreach($user in $sec_staff) {
    if($o_staff -notcontains $user){
      # user was removed since last sync
      "remove " + $user
      Remove-DistributionGroupMember $sec_staff_address -Member $user -Confirm:$false -BypassSecurityGroupManagerCheck
      $n += 1
    }
  }
  $m = 0
  foreach($user in $o_staff) {
      if($sec_staff -notcontains $user){
        # user was added since last sync
        "add " + $user
        Add-DistributionGroupMember $sec_staff_address -Member $user -BypassSecurityGroupManagerCheck
        $m += 1
      }
  }
